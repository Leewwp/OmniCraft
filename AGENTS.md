...
| 4 | grilling 2026-08-08 moderation 批次（执行追踪：GitHub issues #111/#112/#113；追踪：#114/#115） | 3 tickets | 混合 | #111 头像审核 heavy（OSS grant 强制 + ImageModeration + 存量迁移）；#112 私信文本审核 light（复用 #109 的 A4 分环境语义，blocked-by #109 依赖已满足、可开始）；#113 反馈附件图片审核 light。无迁移编号占用；#111 需重试/审计语义与 #104 配置面不重叠 |
| 5 | `docs/superpowers/plans/2026-08-11-omnicraft-reliable-async-observability-foundation-roadmap.md`（执行追踪：冻结门 #134；T00/T02/T03/T08 = #135/#137/#138/#143） | 4 tickets | heavy | 全部被 Web-only 冻结门 #134 阻塞；#134 原生等待 #76/#111/#112/#113。顺序：#135 治理；#137→#138；#143 等 #135/#138/#142。独立 `cmd/worker` 是唯一 Worker 模型；固定迁移 070；改 config/routes/migrations 后跑 doc-validator `--fix` |
...
跨计划可执行 DAG 以 GitHub 原生 `blocked_by` 为机器可读门，本文为人类摘要：`#65` →（`#66/#67/#70/#82` 与媒体 `#83` 按文件并行）→ `#68` 与媒体 `#81/#84/#85~#89` → source-linkage `#96`（已完成）→ 媒体 `#90`（已完成）→ collaboration-invites `#97`（已完成）→ Web `#71` → `#69` → `#72` → `#73` → `#74/#75/#76`。其中 #71/#69/#72 共享筛选、ContentDetail 与 i18n 文件，必须按该顺序串行；Web/safety 末端 #76/#111/#112/#113 全部关闭后才关闭 #134，之后执行 #135~#150 的原生 DAG。发现本文与 GitHub 原生边不一致时先停止并修正二者，不自行选择较宽松的一方。
