...
| 优先级 | 计划文件 | 执行单元 | 车道 | 备注 |
|--------|----------|------|------|------|
| 1 | `docs/superpowers/specs/2026-08-07-omnicraft-web-experience-corrections-design.md`（执行追踪：GitHub issues #64/#65~#76；heavy 细化：`docs/superpowers/plans/2026-08-08-omnicraft-ip-visit-history.md`、`docs/superpowers/plans/2026-08-08-omnicraft-favorites-cutover.md`） | 12 tickets | 混合 | #65 先统一 #64/#80 与 `ui-spec` 权威；#66/#67/#70 可在其后按文件预约并行，#68 承担 C2+C6 的 ContentDetail 收敛和共享 overlay hook 后再接正式转场。媒体显示链完成、source-linkage/#90/collaboration-invites 串行合并后，#71/#69/#72 无业务硬依赖但共享文件须预约串行，三票全部完成后再执行 #73 heavy 独立迁移 `066_ip_visit_history.sql`；#74→#75→#76，#76 为 heavy `067_drop_legacy_favorites.sql` 且在云端日志+可恢复备份人工门满足前标 `ready-for-human`。改 config/routes/migrations 后跑 doc-validator `--fix` |
| 2 | `docs/superpowers/specs/2026-08-08-omnicraft-media-experience-design.md`（执行追踪：GitHub issues #80/#81~#90） | 10 tickets | 混合 | T03 (#83) 改为 heavy 数据/发布合同，迁移 `063_content_media_metadata.sql` 同时落 cover 宽高与附件稳定排序；T04 (#84) 为依赖 #83/#65 的 light 上传编排 UI。#81 还依赖 #66；#85 依赖 #68/#83/#65；#86←#85、#87←#83/#65、#88←#85、#89←#85/#88。#90 在 source-linkage #96 后执行，复用最终 related-fanworks 合同，不在实现时再临时发明数据源。改 config/routes/migrations 后跑 doc-validator `--fix` |
| 3 | `docs/superpowers/plans/2026-06-30-omnicraft-community-source-linkage.md`（执行追踪：GitHub issue #96） | 9 tasks | mixed | 未开始；媒体 T01~T09 的共享面收口后执行；迁移为 `064_add_source_fanwork_id.sql`，Task 1 按 heavy 独立 worktree/提交，其余任务按计划逐任务 checkpoint。与 collaboration-invites 共享 `content_repo.go`、`PublishForm.tsx`、`zh.json`/`en.json`，本计划先合并；#90 在本计划完成后消费最终来源/衍生作品合同 |
...
| 4 | Load gate 修复（执行追踪：GitHub issue #101） | 1 ticket | light | pre-existing 失败（load seed 无 approved content），非 branch protection required check，不阻塞合并但属发布性能门；修复后 CI Load gate 连续 ≥1 次绿 |
| 4 | `docs/superpowers/plans/2026-07-16-omnicraft-web-agent-productization.md` | 6 steps | mixed | Task 1/2/3/5 已合并（PR #35/#37/#39/#41，契约→编排→流式配额→评测门，两阶段审查 blocking 项已修复）；Task 4 已合并（PR #43，旧 Widget 挂载移除、typed normalizer、feature 门控，feature ON/OFF 双态验证）；三缺口 #16/#17/#18/#19 先行落地（PR #36/#38/#40/#42，共享浮窗/推荐页/全站接线/Agent 工作台）。Task 6 已部分完成（分支 `web/agent-t6`，2026-08-08）：Step 1 门禁已勾选，Step 4 截图 4/5（degraded-search 无 UI 差异，留待 Step 2）。MiniMax provider 连通性已验证（私有 embedding wire `texts/type=query`/`vectors`、流式无 `[DONE]` 终止、`<think>` 跨 chunk 剥离），但现有 `agent-smoke` 未经过 AgentService/工具/引用/降级编排，**不计 Step 3 完成证据**；同时修复 config.go viper AutomaticEnv 遮蔽、部署 override 优先级和前端流式 CSRF/auth 恢复。**剩余 Step 2/3/5/6 未勾选**：Step 2 按与 #64 契约共享面协调建议在 #64 T03/T04 浮窗转场落地后执行，之后执行真实 Agent/API smoke；仓库默认 `agent.web_agent_enabled=false` 保持关闭，生产启用仍需 Step 2/3/5/6 完成后单独裁决 |
