...
| 6 | 2026-08-16 双轴审查修复批次（执行追踪：父票 #193；子票 #194~#203） | 10 tickets | 混合 | 2026-08-16 对 `4c3293e..HEAD` 双轴审查（Standards/Spec 并行子代理）后开批次收口，无高危项。**P0 环境** #194（dev DB 迁移 4→69 + 分支清理）；**P1 正确性** #195（submit_ai_review 重投幂等：`sync:<消息ID>` 合成键，heavy TDD）→ #196/#197（消费语义收口、suggestion 单一来源，共享文件串行于 #195）；**P2 整洁** #198（OSS/seed helper 收敛）、#199（ctx 传播 + SetNX 死代码）；**P3** #200（relay 参数 config 化，heavy）、#201（A4 审核门单一实现，heavy）；**P4** #202（published→under_review 降级裁决，`ready-for-human`）、#203（indexer 投影契约基准登记）。机器可读 DAG：`#196/#197` 原生 `blocked_by` `#195`；其余无阻塞可立即开工 |
