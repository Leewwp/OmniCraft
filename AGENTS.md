...
| 0 | `docs/working/2026-09-01-ip-detail-refactor-handoff.md`（设计交接 + 事实索引；执行追踪：GitHub #290，45 条 user stories） | 1 ticket（+T17 #237 收窄票同窗） | heavy（迁移 073+，共治提案新域） | 2026-09-01 贴吧式重构设计定稿（三轮 R1~R3 拍板）+ 交互原型已交付（`/prototype/ip-detail-hub`，落地后移除）。单页 query 驱动三模块（分享/讨论/提案投票）+ 共治提案独立域（min_votes=10/pass_threshold=0.6/deadline 7d）+ IP 内搜索 + 置顶权收归系统管理员。吸收 FIX-25 可见性部分 / F-064 / 类目页死排序（mod label 由 U-04 先修、此处仅校验）；不吸收 FIX-18/FIX-23。✅ 执行冻结已解除（2026-09-02），位于总序第 5 段（U 批次之后） |
