...
| 优先级 | 计划文件 | 余项 | 车道 | 备注 |
|--------|----------|------|------|------|
| 1 | `docs/superpowers/plans/2026-07-18-omnicraft-ui-polish-hardening.md` | U-10→U-11→U-12 | light | U-05/U-02A/U-04/U-01/U-02B/U-03/U-06/U-07/U-08/U-09 已完成；P-01 已于 2026-08-01 获批（IP 库选定方案 B Indigo 精修），采用当前 Indigo 单方向，Hallmark Tally 方向已取消。A1/A1.5/A1.6/A2 已完成并验证。生产 `/ips` 的固定卡宽空白缺陷已由 U-09 修复。现有二创页 `/`、原创页 `/original`、IP 详情 `/ip/[ipId]` 为已接受视觉基线，不重复构建。前端任务由 opencode 车道执行；U-11 后端能力与 U-12 的 verifier 共享文件按现有协调规则与 Production Readiness 串行；真实图片与全历史搜索生产接线另走后续计划 |
...
| 4 | `docs/superpowers/plans/2026-07-16-omnicraft-web-agent-productization.md` | 47 | mixed | 未开始；前端 Task 4 等待 P-01→U-01→U-02B→U-03，生产 `/agent`、旧 Widget 移除和引用浮层接线由本计划独占；真实 Provider 密钥缺失时按阻塞处理，仓库默认开关保持关闭 |
