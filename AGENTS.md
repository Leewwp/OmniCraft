...
| 优先级 | 计划文件 | 余项 | 车道 | 备注 |
|--------|----------|------|------|------|
| 2 | `docs/superpowers/plans/2026-07-17-omnicraft-production-readiness.md` | 44（其中 9 项桌面暂缓） | heavy | Ops-00、Ops-01、Ops-02、Ops-03 已完成；Ops-03 经内部 metrics 端口/readiness/脱敏日志契约、Prometheus/Loki/Alloy 实盘 drill 与 project gate 验证，Web 发布路径下一任务为 Ops-04（需真实独立故障域 HTTPS heartbeat 后完成），之后继续 Ops-05~Ops-08。2026-08-03 起计划内脚本统一为 bash（`.sh`），本地与 CI 均用 `bash <path>` 调用，不再引用 `.ps1`/pwsh。桌面制品 Ops-09 随桌面范围暂缓，不得复制或绕过桌面安全任务 |
