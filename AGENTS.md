...
| 优先级 | 计划文件 | 余项 | 车道 | 备注 |
|--------|----------|------|------|------|
| 1 | `docs/superpowers/specs/2026-08-07-omnicraft-web-experience-corrections-design.md`（执行追踪：GitHub issues #64/#65~#76） | 12 tickets | 混合 | Ticket 01~12 全部 ready-for-agent 未开始；按依赖串行（2026-08-08 与媒体计划交错确认，见 docs/working/2026-08-08-architecture-fix-backlog.md）：T01→T02→T06（UI 高收益修复，light，可与媒体 T01/T02/T03 并行）→ **提前合并 T03/T04**（浮窗转场/媒体加载，媒体 T05 的硬前置）→ 媒体体验计划 T05~T10（见优先 2 行）→ source-linkage 计划（共享 ContentDetail/路由/翻译，先合并）→ T07→T05→T08（Reaction/目录/排序，light）→ T09 IP history 独立表（heavy，独立迁移 `064_ip_visit_history.sql`）→ T10→T11→T12 收藏集 cutover（heavy，`065_drop_legacy_favorites.sql`；T12 需云端人工门：访问日志确认无旧端点调用 + 可恢复迁移前备份）。改 config/routes/migrations 后跑 doc-validator `--fix` |
| 2 | `docs/superpowers/specs/2026-08-08-omnicraft-media-experience-design.md`（执行追踪：GitHub issues #80/#81~#90） | 10 tickets | 混合 | Ticket 01~10 全部 ready-for-agent 未开始；原生阻塞边：#83(T03)→#84(T04)→#85(T05)；#86(T06)←#85；#87(T07)←#84；#88(T08)←#85；#89(T09)←#85/#88；#90(T10)←#88；T01/T02/T03 无阻塞可先行。与 #64 共享 ContentDetail/ContentDetailOverlay/ContentCard/媒体面与 zh.json/en.json，串行规则（2026-08-08 确认交错）：T01/T02/T03 可与 #64 T01/T02/T06 并行，媒体 T05~T10 在 #64 T03/T04 合并后进行（#64 行同注）；i18n 文件与 source-linkage/collaboration-invites 串行，执行序在 #64 之后、source-linkage 之前。迁移 `067_content_cover_dimensions.sql`（heavy：T04；067 将先于 063~066 应用，runner 按版本升序补缺可安全处理，但 T04 须先落「非连续 migration fixture 验证门」——见 docs/working/2026-08-08-architecture-fix-backlog.md C7 与 issue #84 验收项）。改 config/routes/migrations 后跑 doc-validator `--fix` |
| 3 | `docs/superpowers/plans/2026-06-30-omnicraft-community-source-linkage.md` | 64 | light | 未开始；迁移编号已修正为 `066_add_source_fanwork_id.sql`（2026-08-07 重编号：`061` 因 `062_notification_broadcast_idempotency.sql` 已先入库而不可回填，`063`/`064`/`065` 已分别钉名 collaboration-invites/IP history（#64 T09）/favorites drop（#64 T12））。与 collaboration-invites 共享 `content_repo.go`、`zh.json`/`en.json`，须串行执行且本计划先合并。与 #64 Web 体验修复共享 ContentDetail/路由/翻译面，须与 #64 串行（#64 的 T01/T02/T06 先合并，或按文件预留协调） |
