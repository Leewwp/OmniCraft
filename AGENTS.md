...
| 优先级 | 计划文件 | 余项 | 车道 | 备注 |
|--------|----------|------|------|------|
| 2 | `docs/superpowers/plans/2026-07-17-omnicraft-production-readiness.md` | 30（其中 9 项桌面暂缓） | heavy | Ops-00～Ops-05 已完成；Ops-04 经 alert 契约测试、promtool/amtool、真实 firing/resolved drill（in-network alert-sink）与真实 Healthchecks.io missing-heartbeat down-flip 验证（已合并至 main `ddaf0a4`，含 llm 语义合并）；Ops-05 经策略/例外/action-pin 契约测试、夹具失败隔离、默认项目门与 filesystem/IaC/image 全量扫描验证（已合并至 main `9c0e195`），Web 发布路径下一任务为 Ops-06，之后继续 Ops-07~Ops-08。2026-08-03 起计划内脚本统一为 bash（`.sh`），本地与 CI 均用 `bash <path>` 调用，不再引用 `.ps1`/pwsh。桌面制品 Ops-09 随桌面范围暂缓，不得复制或绕过桌面安全任务 |
