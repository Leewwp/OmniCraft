...
| 优先级 | 计划文件 | 余项 | 车道 | 备注 |
|--------|----------|------|------|------|
| 2 | `docs/superpowers/plans/2026-07-17-omnicraft-production-readiness.md` | 29（其中 9 项桌面暂缓） | heavy | Ops-00～Ops-07 已完成；Ops-04 经 alert 契约测试、promtool/amtool、真实 firing/resolved drill（in-network alert-sink）与真实 Healthchecks.io missing-heartbeat down-flip 验证（已合并至 main `ddaf0a4`，含 llm 语义合并）；Ops-05 经策略/例外/action-pin 契约测试、夹具失败隔离、默认项目门与 filesystem/IaC/image 全量扫描验证（已合并至 main `9c0e195`）；Ops-06 经 manifest/SBOM/pinned-generator 契约测试、确定性两轮对比、17 项 provenance 验证（OCI label↔commit 绑定）、下载复验、一年保留期归档 receipt 与默认项目门验证（已合并至 main `26dc41b`）；Ops-07 经 runner/target-safety 契约测试、批准的 release performance profile、真实 smoke/load/stress 基线测量与 k6 阈值验证（已合并至 main `a219a59`）；**Ops-08 已完成**：Step 1-4 契约门 + Step 5 真实 staging 演练（7 项真实输入全部到位：omnicraft OSS 桶 cn-guangzhou versioning、服务器实读 Green/CAPTCHA 凭据、GitHub Release v0.1.0-rc.1、off-site omnicraft-ops-archive；真实容器 deploy/rollback/redeploy、真实 pg_dump/restore 与阿里云 OSS 版本恢复实测、RPO≤5min/RTO≤30min 批准目标 machine-compare 全达标、双目的地归档 receipt verified；已合并至 main `074715f`）。仅剩 Ops-09 随桌面范围暂缓。2026-08-03 起计划内脚本统一为 bash（`.sh`），本地与 CI 均用 `bash <path>` 调用，不再引用 `.ps1`/pwsh。桌面制品 Ops-09 随桌面范围暂缓，不得复制或绕过桌面安全任务 |
