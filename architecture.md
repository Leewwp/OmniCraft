...
GET    /api/v1/admin/appeals          # 申诉列表
POST   /api/v1/admin/appeals/:id      # 处理申诉
...
CREATE TABLE reputation_logs (
    ...
    -- reason: 'malicious_report' | 'malicious_comment' | 'malicious_contribution'
    --         | 'malicious_report_comment' | 'judge_error' | 'quality_content' | etc.
...
CREATE TABLE ips (
    ...
    description     TEXT,
    cover_url       TEXT,
    category        VARCHAR(50),   -- '动漫' | '游戏' | '影视' | '音乐' | '其他'
...
CREATE TABLE content_items (
    ...
    -- content_type: 'article' | 'image' | 'gif' | 'video' | 'audio'
...
CREATE TABLE browse_history (
    id              BIGSERIAL PRIMARY KEY,
    user_id         BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    target_type     VARCHAR(20) NOT NULL,  -- 'content' | 'ip'
    target_id       BIGINT NOT NULL,
...
CREATE TABLE judge_questions (
    ...
    question_data   JSONB NOT NULL,        -- 题目内容（含选项、正确答案）
    is_active       BOOLEAN NOT NULL DEFAULT TRUE,
...
CREATE TABLE judge_cases (
    ...
    min_votes       INT NOT NULL DEFAULT 5,   -- 最小投票数才生效
...
- **角色权限**：`user` / `creator` / `judge` / `admin`，中间件层拦截
