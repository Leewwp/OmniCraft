...
if rg -q 'REPORT_DIR/alertmanager\.runtime\.yml' "$DRILL"; then
...
if rg -q '"url".*config\["ping_url"\]|temp_check_uuid|temp_check_channels|"check": created' "$DRILL"; then
  echo "FAIL: archived heartbeat JSON must omit URLs, UUIDs and channel identifiers" >&2
  exit 1
fi
rg -q 'mktemp -d.*omnicraft-alert-drill-runtime' "$DRILL" \
  || { echo "FAIL: runtime credentials must use a private temporary directory" >&2; exit 1; }
rg -q 'teardown_status=\$?' "$DRILL" \
