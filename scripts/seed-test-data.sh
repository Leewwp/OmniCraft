...
# Exit codes:
#   0 - success
#   1 - login failed or API error on critical operation

set -euo pipefail

BASE="${SEED_BASE_URL:-http://localhost:8080/api/v1}"
SEED_EMAIL="${SEED_EMAIL:-demo@omnicraft.com}"
SEED_PASSWORD="${SEED_PASSWORD:-demo123456}"
CT="Content-Type: application/json"

# --- Helper: login and obtain token ---
echo "=== Logging in ==="
LOGIN_RESP=$(curl -sf -X POST "$BASE/auth/login" -H "$CT" \
  -d "{\"email\":\"$SEED_EMAIL\",\"password\":\"$SEED_PASSWORD\"}" 2>/dev/null) || {
  echo "FATAL: login failed for $SEED_EMAIL" >&2
  exit 1
}
TOKEN=$(echo "$LOGIN_RESP" | python3 -c "import sys,json; print(json.load(sys.stdin)['tokens']['access_token'])" 2>/dev/null) || {
  echo "FATAL: could not extract access_token from login response" >&2
  exit 1
}
AUTH="Authorization: Bearer $TOKEN"
echo "Logged in successfully"

# --- Helper: POST with status check ---
api_post() {
  local label="$1"
  local path="$2"
  local body="$3"
  local tmp=$(mktemp)
  local http_code
  http_code=$(curl -s -o "$tmp" -w "%{http_code}" -X POST "$BASE$path" -H "$AUTH" -H "$CT" -d "$body") || {
    echo "FATAL: curl failed for $label" >&2; rm -f "$tmp"; exit 1
  }
  if [ "$http_code" -ge 400 ]; then
    echo "FATAL: $label returned HTTP $http_code" >&2
    cat "$tmp" >&2
    rm -f "$tmp"
    exit 1
  fi
...
extract_id() {
  echo "$1" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('id', d.get('content',{}).get('id','')))" 2>/dev/null
}
...
# Original 1: 影视 - 图文
OC1=$(api_post "Create original 流浪地球剧照分析" "/contents" '{
...
OC1_ID=$(extract_id "$OC1")
echo "Created original 流浪地球剧照分析: id=$OC1_ID"

# Original 2: 游戏 - 图文
OC2=$(api_post "Create original 黑神话攻略" "/contents" '{
...
OC2_ID=$(extract_id "$OC2")
echo "Created original 黑神话攻略: id=$OC2_ID"

# Original 3: 美食 - 图文
OC3=$(api_post "Create original 美食饭团" "/contents" '{
...
OC3_ID=$(extract_id "$OC3")
echo "Created original 美食饭团: id=$OC3_ID"

# Original 4: 宠物 - 图文
OC4=$(api_post "Create original 橘猫日记" "/contents" '{
...
OC4_ID=$(extract_id "$OC4")
echo "Created original 橘猫日记: id=$OC4_ID"

# Original 5: 文学 - 纯文字
OC5=$(api_post "Create original 科幻短篇" "/contents" '{
  "title": "短篇科幻 | 《最后的信使》",
...
OC5_ID=$(extract_id "$OC5")
if [ -z "$OC5_ID" ]; then echo "FATAL: failed to extract OC5 ID" >&2; exit 1; fi
echo "Created original 科幻短篇: id=$OC5_ID"

# Original 6: 旅行 - 图文
OC6=$(api_post "Create original 川藏线攻略" "/contents" '{
...
OC6_ID=$(extract_id "$OC6")
echo "Created original 川藏线攻略: id=$OC6_ID"

# Original 7: 数码科技 - 视频
OC7=$(api_post "Create original MacBook评测" "/contents" '{
...
OC7_ID=$(extract_id "$OC7")
echo "Created original MacBook评测: id=$OC7_ID"

# Original 8: 家居 - 图文
OC8=$(api_post "Create original 出租屋改造" "/contents" '{
...
OC8_ID=$(extract_id "$OC8")
echo "Created original 出租屋改造: id=$OC8_ID"

# Original 9: 美妆穿搭 - 图文
OC9=$(api_post "Create original 穿搭分享" "/contents" '{
...
OC9_ID=$(extract_id "$OC9")
echo "Created original 穿搭分享: id=$OC9_ID"

# Original 10: 运动 - 效率模板
OC10=$(api_post "Create original 马拉松模板" "/contents" '{
...
OC10_ID=$(extract_id "$OC10")
if [ -z "$OC10_ID" ]; then echo "FATAL: failed to extract OC10 ID" >&2; exit 1; fi
echo "Created original 马拉松模板: id=$OC10_ID"

# Original 11: 效率 - 纯文字
OC11=$(api_post "Create original 知识管理" "/contents" '{
...
# Fanwork 1: 原神同人插画
FW1=$(api_post "Create fanwork 雷电将军插画" "/contents" "{
...
FW1_ID=$(extract_id "$FW1")
echo "Created fanwork 雷电将军插画: id=$FW1_ID"

# Fanwork 2: 星穹铁道同人文 - 绑定原创
FW2=$(api_post "Create fanwork 星穹同人文" "/contents" "{
...
FW4_ID=$(extract_id "$FW4")
echo "Created fanwork 阿米娅插画: id=$FW4_ID"

# Fanwork 5: 原神AI提示词
FW5=$(api_post "Create fanwork 胡桃LoRA" "/contents" "{
...
FW5_ID=$(extract_id "$FW5")
echo "Created fanwork 胡桃LoRA: id=$FW5_ID"

# Fanwork 6: 流浪地球Mod
FW6=$(api_post "Create fanwork 流浪地球MOD" "/contents" "{
...
FW6_ID=$(extract_id "$FW6")
echo "Created fanwork 流浪地球MOD: id=$FW6_ID"

# Fanwork 7: 鬼灭之刃同人乐谱
FW7=$(api_post "Create fanwork 鬼灭钢琴谱" "/contents" "{
...
FW7_ID=$(extract_id "$FW7")
if [ -z "$FW7_ID" ]; then echo "FATAL: failed to extract FW7 ID" >&2; exit 1; fi
echo "Created fanwork 鬼灭钢琴谱: id=$FW7_ID"

# Fanwork 8: 星穹铁道二创图
FW8=$(api_post "Create fanwork 银狼插画" "/contents" "{
