#!/usr/bin/env bash
# Contract tests for scripts/db/redis-reconciliation-drill.sh. The real drill requires
...
DRILL="$SCRIPT_DIR/redis-reconciliation-drill.sh"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
COMPOSE="$REPO_ROOT/ops/recovery/docker-compose.recovery.yml"

if [ ! -f "$DRILL" ]; then
  echo "redis-reconciliation-drill.sh does not exist" >&2
  exit 1
fi
if [ ! -f "$COMPOSE" ]; then
...
TEMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/omnicraft-redis-drill.XXXXXX")"
...
if grep -Fq 'down -v --remove-orphans >/dev/null 2>&1 || true' "$DRILL"; then
  echo "FAIL: redis reconciliation drill must propagate teardown failure" >&2
  exit 1
fi

echo "redis-reconciliation-drill contract tests passed"
