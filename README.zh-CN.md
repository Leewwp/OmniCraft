...
## 评测基建与 PR 级零成本门禁（SP-22）

检索层指标是纯 ID 对账（golden 集 vs 检索返回 ID），不调任何 provider，毫秒级跑完——因此它不该是"偶尔跑一次的报告"，而应是**每个 PR 都跑的门禁**。

- **PR 级零成本门禁**：`evals/thresholds.yaml` 定义 `minimums` / `maximums` / `integrity` 三段阈值，`backend/cmd/rag-gate` 对冻结快照执行断言，本地运行 < 0.1s；红绿双路径与 exit 2（配置错误）分流已实证；`integrity` 段校验快照非空与元数据完整，堵住"空快照假绿"。
- **196 条生成层基线**（`label=sp22-e1-baseline-dev`，hybrid=on / expansion=off / rerank=off，152/196 参与检索层）：context recall@10 0.9605、hit@5 0.9539、MRR 0.9138；拒答混淆矩阵（可答 176 / 不可答 20）：可答正确率 0.9545、over-refusal 0.0455、**幻觉率 0.9000 为已定位观察项（未修复）**、拒答正确率 0.1000。Ragas 口径生成层 harness（judge=DeepSeek，896 次判官调用 ≈1.41M token）：faithfulness 0.792 / answer relevancy 0.716 / context precision 0.662 / noise sensitivity 0.021。
- **judge 校准**：18 条 × 3 次重跑，faithfulness mean|Δ| 0.187、同侧率 82.4%、Pearson 0.808；answer relevancy Pearson −0.174——**据此禁用该指标的门禁资格**（仅保留观测）。真值为模型标注（非人类标注）。
- **归因网格**：8 配置首扫发现 `rrf_k` 与候选池大小为零灵敏度轴；rerank 是唯一全指标正贡献格（recall@10 0.9833→0.9917、MRR 0.9338→0.9833、延迟 +206ms）。

> **两套口径不得混用**：上一节差分口径 = current-v1 冻结语料 63 cases、真实 provider 运行（偏质量诊断）；本节快照口径 = 196 条 ID-based 零成本快照（偏回归门禁）。报数时必须说清是哪一套、什么配置、什么日期。评测草稿绝不自动进入冻结集，必须人工策展。
...
已完成**单机生产部署**（[app.leeppp.online](https://app.leeppp.online) / api.leeppp.online，3.6 GiB 精简档；runbook：[docs/deploy/single-server-beta-runbook.md](docs/deploy/single-server-beta-runbook.md)，Status: PRODUCTION；部署/回滚演练证据归档于 `artifacts/ops-08/`）。**部署完成 ≠ 上线运营**：无公开运营、无真实用户、无 QPS/SLA 数据。当前默认配置与边界：
