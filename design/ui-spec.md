...
**共享元素转场（#64 决策 7-10 权威，覆盖 #67 原型与 #68 接入；#398 C1~C4 契约落地）**
...
- **C1 图源统一**：卡片封面与浮窗封面（MediaGallery/竖屏集媒体列）同走 next/image 优化器变体（SVG 直通=两端同 URL 亦达标）；点击卡片瞬间预取浮窗显示宽度变体并 decode；MediaViewer 看大图维持原图。
