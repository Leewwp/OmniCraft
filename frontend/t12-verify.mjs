...
// —— 场景 1+2：norm 同 context ——
{
  const ctx = await browser.newContext({ viewport: { width: 1440, height: 900 } });
  const page = await ctx.newPage();
  const loggedIn = await login(page, "lane-s-norm@seed.omnicraft.local", "LaneS-Norm#2026");
  expect(loggedIn, "norm 登录成功（me 200）");

  // 场景 1：发帖
  await page.goto(`${BASE}/ip/4200?tab=discussions`, { waitUntil: "domcontentloaded" });
  const newBtn = page.locator('a:has-text("发起讨论")').first();
  await newBtn.waitFor({ state: "visible", timeout: 15000 }).catch(() => {});
  await Promise.all([
    page.waitForURL(/discussions\/new/, { timeout: 10000 }).catch(() => {}),
    newBtn.click(),
  ]);
  await page.fill("#discussion-title", "浏览器回归帖 T12");
  await page.fill("#discussion-body", "正常用户经前端发帖走统一审核门（回归）。");
  await Promise.all([
    page.waitForURL(/tab=discussions/, { timeout: 15000 }).catch(() => {}),
    page.click('button[type="submit"]'),
  ]);
  await page.waitForTimeout(1500);
  const listed = await page.locator('text=浏览器回归帖 T12').first().isVisible().catch(() => false);
  expect(listed, "新帖出现在讨论列表（201 路径回归）");
  await shot(page, "t12-norm-posted");

  // 场景 2：拿新帖 id → ?d= 直开浮层 → 回复
  const discId = await page.evaluate(async () => {
    const r = await fetch("http://localhost:8094/api/v1/ips/4200/discussions?page=1&page_size=5");
    const d = await r.json();
    const hit = (d.discussions || []).find(x => x.title === "浏览器回归帖 T12");
    return hit ? hit.id : 0;
  });
  expect(discId > 0, "取回新帖 id=" + discId);
  await page.goto(`${BASE}/ip/4200?tab=discussions&d=${discId}`, { waitUntil: "domcontentloaded" });
  const replyBox = page.getByPlaceholder("写下你的回复...").first();
  await replyBox.waitFor({ state: "visible", timeout: 20000 }).catch(() => {});
  if (await replyBox.isVisible().catch(() => false)) {
    await replyBox.fill("浏览器回归回复（content 字段映射）");
    await replyBox.press("Enter");
    await page.waitForTimeout(1500);
    const replyShown = await page.locator('text=浏览器回归回复（content 字段映射）').first().isVisible().catch(() => false);
    expect(replyShown, "回复出现（content→body 映射不 400）");
  } else {
    expect(false, "回复输入框不可见（浮层/会话未就绪）");
  }
  await shot(page, "t12-norm-reply");
  await ctx.close();
}

// —— 场景 3：low 提交 → 403 用户文案 + 无落库 ——
{
  console.log("waiting 70s for login rate-limit window...");
