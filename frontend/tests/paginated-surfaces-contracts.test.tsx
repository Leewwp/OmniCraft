...
const paginatedPageContracts = [
  ...
  ["IP discussions", "app/(public)/ip/[ipId]/discussions/page.tsx", /page=\$\{nextPage\}/],
] as const;
...
test("paginated pages own cursor state and reset the first page for filters/search", () => {
  ...
  const discussions = read("app/(public)/ip/[ipId]/discussions/page.tsx");

  assert.match(history, /void load\(1, false\)/);
  assert.match(history, /page_size: "20"/);
  assert.match(discussions, /page=\$\{nextPage\}/);
  assert.match(discussions, /load\(\)/);
});
