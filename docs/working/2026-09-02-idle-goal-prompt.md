...
**当前 checkpoint（2026-09-02 午间刷新，#290 进行中）**：固定范围共 69 个执行单元（含修订一新增的语料 v2 #291），已有 18 个以 GitHub `CLOSED/COMPLETED` 收口（T01~T09、T18~T20、T22、U-01~U-05）。**#290（IP 详情页贴吧式重构，heavy）已开工未收口**：worktree `/Users/pp/Desktop/file/code/project/OmniCraft-heavy-290`（分支 `codex/heavy/290`，基点 456339f）上有约 40 个文件的未提交改动——后端已完成（提案域 TDD 绿、迁移 073、路由接线与 container 注册、hub stats、热议排序 q/hot/置顶、通知 fanout、doc-validator、`go test ./...` 全绿）；前端已完成（`components/ip/hub/` 组件齐、`app/(public)/ip/[ipId]/page.tsx` 重建、旧路由与 IPDetail*/prototype 删除、next.config 301 重定向、i18n hub*/proposal 键、4 个契约测试更新）；**剩余步骤：重写 e2e spec `frontend/e2e/ip-detail-category.mock.spec.ts` 到新 Hub 契约（单页 + `?tab=` query，旧 category/discussions 路径已删，0% 完成）→ 前端 tsc/typecheck → 前端全量测试 → 浏览器验证与截图 → 验证门全量 → 两阶段审查 → 主 worktree 合并 → 关票**。恢复时先 `git -C /Users/pp/Desktop/file/code/project/OmniCraft-heavy-290 status` 对账：状态与上述吻合 → 先打一个 WIP commit 保护现场再继续（§2.7）；差异较大 → 以实际对账结果判断已完成部分，不重做。#290 收口后同域紧随 T17（#237 收窄版）。本段仅为导航快照，计数与完成判断以总序 §1.1 与 GitHub 实际状态为准。
