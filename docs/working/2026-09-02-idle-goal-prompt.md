...
> 创建日期：2026-09-02 ｜ 预计失效日期：整合总序全部落地后
...
- 状态三源合一：GitHub 票据状态（机器可查）+ 总序 §3 的 ✅ 标记（人类可读）+ `progress.txt`（过程记录）。三者不一致时以 GitHub 票据为准并修正其余两处。
...
4. `gh -R Leewwp/OmniCraft issue list --state open --limit 100` 对照总序 §3，定位第一个「未 ✅ 且前置已满足」的票（前置 = 票内 `## Blocked by` 引用的票均已 closed，且总序段序在其后）。
5. 从该票继续。**禁止重做任何已 ✅ / 已 closed 的工作**；若发现已 ✅ 项存在回归，作为新缺陷记录（progress.txt + 总序备注），排入当前段末尾处理，不回滚整段。
...
3. **起环境**：`docker compose up -d postgres redis`（镜像拉取失败用 docker.m.daocloud.io 代理，本机无法直连 Docker Hub）；`cd backend && go run cmd/server/main.go &`；`cd frontend && npm run dev &`；确认 :8080 / :3000 可达再动手。
...
7. **记录与提交**：`progress.txt` 追加条目（What was done / Testing / Notes）；总序 §3 对应条目追加 ✅ 与 commit hash；`git add <精确文件列表>` 提交。heavy 票一票一分支一 commit（`git checkout -b codex/heavy/<票号>` 起，审查通过后 merge 回 main）；light 票在 main 上按逻辑分批 commit。提交信息用 conventional 风格（feat/fix/docs/test）。
8. **关票**：`gh -R Leewwp/OmniCraft issue close <N> -c "完成于 <hash>；验证证据：<一句话摘要（测试结果/截图说明）>"`。

## 3. 顺序与并行协议

- 默认严格串行，按总序 §3 八段顺序取票；段内顺序自由但必须尊重票内 `## Blocked by`。
...
- 单票阻塞（外部服务/凭证/迁移冲突/契约冲突）：按 AGENTS.md「⚠️ 阻塞处理」输出阻塞块写入 progress.txt，**跳过该票继续总序下一票**（标注 `blocked`，留待人工），不死等、不阻塞整条流水线。
...
总序 §3 八段全部 ✅（= 简历投递门槛达成）后：汇总各段 commit 区间与验证证据写入 progress.txt，输出「整合总序执行完毕」总结，结束。剩余 open 票若全部为 blocked/外部输入待补，同样视为本轮收口。
