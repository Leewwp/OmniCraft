...
本 goal 的执行范围固定为整合总序 §3 的执行单元：T01–T55（#221~#275）、U-01~U-05（#277~#281）、A-01~A-07（#283~#289）、#290 和语料 v2（CORPUS-01，#291，2026-09-02 修订新增）。语料 v2 的语料文本为外部输入（独立会话生成）：未就绪时按 §5 阻塞协议保持 OPEN 并跳过，禁止以 mock 语料入库。#276/#282 仅为协调父票，不实现、不关票。活计划中的 #204 面试证据收口、#32 简历与 live-demo 叙事、#207 已完成基线以及 #208 Phase 2 roadmap 不在本 goal 内；不得因它们仍为 OPEN 就擅自扩展范围。

**当前 checkpoint（2026-09-02，第 4 段收口后）**：固定范围共 69 个执行单元（含修订一新增的语料 v2 #291），已有 18 个以 GitHub `CLOSED/COMPLETED` 收口（T01~T09、T18~T20、T22、U-01~U-05），剩余 51 个。用户摘要中的“14/45”不对应本 goal 的固定分母；计数和完成判断必须以总序 §1.1 与 GitHub 实际状态为准。第 3、4 段均已收口（各自 verify-project.sh --full 通过）。下一张可执行票是第 5 段 **#290（IP 详情页贴吧式重构，heavy 车道：一任务一 worktree + TDD + 两阶段审查；迁移 073+）**，完成后同域紧随 T17（#237 收窄版）。不得重做已关闭的 18 张票。
