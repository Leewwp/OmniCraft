...
7. **记录与提交**：`progress.txt` 追加条目（What was done / Testing / Notes）；只有验证门全过才在总序 §3 对应条目追加 ✅ 与 commit hash；`git add <精确文件列表>` 提交。heavy 票须从干净基线创建独立 worktree：`git worktree add /Users/pp/Desktop/file/code/project/OmniCraft-heavy-<票号> -b codex/heavy/<票号> <当前基线>`，一票一分支；**闲时模式下 heavy 分支允许并要求按里程碑打 WIP commit**（`wip(#N): 里程碑`，如「后端测试绿」「前端 tsc 绿」「契约测试更新完」，每 ≤1 小时或每个大阶段至少一个）——这是防止票据墙硬杀丢失数小时工作的生存性例外（2026-09-02 用户批准）；「一任务一 commit」放宽为「一任务一合并」，收口时可 squash WIP 或保留；**合并一律走 PR（2026-09-03 Phase 2 起，main 已开分支保护 required check `project-gate` + `enforce_admins`，直推 main 必被拒）**：heavy 票两阶段审查通过后 push 分支（非交互环境用内嵌 token：`git push "https://x-access-token:$(gh auth token)@ghfast.top/https://github.com/Leewwp/OmniCraft.git" <branch>`）→ `gh pr create --repo Leewwp/OmniCraft` → `gh pr merge --squash --auto --repo Leewwp/OmniCraft`（CI project-gate 绿后自动合并）；合并后回主 worktree `git pull` 同步并清理 worktree/分支。light 票同样走 feature 分支 → PR → squash 合并（分支上保留逻辑分批 commit）。**旧分支规则**：分支基线早于 ci.yml PR 触发器合入点（0d34757，2026-09-02）的，开 PR 前须先 sync main，否则 PR 不触发 CI。禁止在主 worktree 直接 `git checkout` 切走用户改动。提交信息用 conventional 风格（feat/fix/docs/test）。
