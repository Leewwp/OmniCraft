...
20. **A-06（#288）** 工作台 DeepSeek 化（前置已全部满足：A-01/A-02/U-02/T18/T19/T20；吸收 U-04 剔除的工作台 sweep 范围；对比度与行缓冲语义不得回退）⏸→待合并：2026-09-03 实现与验证门全过——分支 feat/a06-agent-workspace-deepseek（SSE v2 消费层/续写契约请求体/三层形态/侧边栏三交互/锚定/markdown/i18n/ui-spec/服务端提示词一行/双渲染修复），单测 470 绿 + mock e2e 13/13（contracts 全量 84/84）+ 后端 build/vet/test 全绿 + 浏览器真跑 17/17（DashScope 真流式逐字/grounded 引用/[1] 锚定聚焦高亮/详情浮层/复制剪贴板实文/重命名/置顶/删除/自动增高 44→208/暗色/窄屏 0 溢出；截图 a06-* 8 张）；PR 合并后本条改 ✅
