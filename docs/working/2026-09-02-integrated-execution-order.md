...
本文档与配套 idle goal 的执行范围固定为本页列出的执行单元：T01–T55（#221~#275）、U-01~U-05（#277~#281）、A-01~A-07（#283~#289）、#290，以及 2026-09-02 修订一新增的**语料 v2（CORPUS-01，#291）**；#276/#282 仅为协调父票，不实现、不关票。活计划中的 #204（面试证据收口）与 #32（简历/live-demo 叙事）仍是独立下游工作，不因本总序完成而自动关闭或代办。
...
- **A 批次**：A-01 #283 / A-02 #284 / A-03 #285 / A-05 #287 均为 frontier；**语料 v2 #291 ← A-03 + 语料文本（外部输入）**；A-04 #286 ← A-03 + **语料 v2 #291**（2026-09-02 修订一）；A-06 #288 ← A-01 + A-02；A-07 #289 ← A-06。A-01 heavy（迁移），其余 light。语料 v2 的语料文本由独立会话生成，未就绪时该票按阻塞协议保持 OPEN 跳过，禁止 mock。
...
- 固定 goal 范围共 **69 个执行单元**（2026-09-02 修订一 +1）：T01–T55（55）+ U-01~U-05（5）+ A-01~A-07（7）+ #290（1）+ 语料 v2 #291（1）。其中 **34 个已完成并在 GitHub 以 `CLOSED/COMPLETED` 收口**：T01~T09（9）、T17~T22（6）、T35（1）、第 8 段 D 批 T23/T45/T46/T54/T25/T47（6，2026-09-03 道 3 收口）、U-01~U-05（5）、#290（1）、A-01/A-02/A-03/A-05/A-06/A-07（6）；剩余 35 个（33 张 T 票 + 语料 v2 #291 + A-04 #286），#276/#282 仍为协调父票不计入。
- **第 3、4、5 段已全部收口**；第 6 段实现票全部完成：A-01（fd4c33e）+ A-02（8517032）+ A-05（6cef43b）+ A-03（619cda9，段边界 verify --full 81/81 通过 2026-09-03）。第 6 段余下 = **语料 v2（#291，语料文本已于 2026-09-03 生成就绪 1600/1600 并进入本地注入执行）→ A-04（#286，blocked_by #291）**。**第 7 段已完成**（T21 ac4f393 + A-06 5010ac7 含 t72 回归修复 f898a08 + A-07 a0fc7da；段边界 verify --full 通过 2026-09-03——全部门 + mocked contracts 84/84，contracts 相因并行注入会话占用 3001 保留端口分两次补齐）。**第 8 段 D 批已由道 3 清票会话收口**（T23 #312 55f3647 / T45 #313 608db3d / T46 #314 3c5ed7c / T54 #315 b719c43 / T25 #317 33540af / T47 #324 1b107b3，每票独立 PR 五门绿 squash）。语料 v2 #291 注入已完成（PR #316 c40faf0，1600/1600 走真实发布管线，worker lag=0），golden set 复核进行中（用户；2026-09-04 审计检查点 PR #327 已入——不构成冻结，semantic 48/48 标题复读待 v2 重写）；冻结后链 = 关 #291 → A-04 解锁（原前置 #319 tsquery 冒号已于 2026-09-04 修复，PR #326）。**2026-09-04 派发**：道 A 产品 bug 批（#319→#318→#320→#321，#323 顺手；#322 并入 T32 执行）+ 道 B 闲时清票（B 批子链 T10/T11/T41/T42/T43/T44 + 备 T53/T55，提示词 = `docs/working/2026-09-04-longtask-laneb-seg8b-prompt.md`）。**道 A 产品 bug 批前四票已于 2026-09-04 收口**（#319 #326 / #318 #328 / #320 #329 / #321 #330，main=7326ad9；#323 CI 闪断残余保持开票，#322 仍并入 T32 执行）；道 B 截至 2026-09-04 晚尚未开工（无 worktree/分支）。
...
11. ✅ **#290** 贴吧式社区枢纽（heavy：迁移 073+、共治提案域、单页三模块、IP 内搜索；吸收可见性/F-064/类目页死排序；mod label 由 U-04 已修则仅校验继承；消费 U-02/U-03 成果；原型 `/prototype/ip-detail-hub` 为脚手架，落地后移除该原型路由）——完成于 6e6d77e（2026-09-02，squash codex/heavy/290；两阶段审查 REJECTED→全必须项修复→增量复审 DONE_WITH_CONCERNS 收口；全量门 EXIT=0 + 浏览器实测 15/15；GitHub `CLOSED/COMPLETED`；记录偏差：新提案通知走 Notify 逐关注者异步扇出而非 outbox 事件，MVP 可接受）
...
- **D 触达反馈**：~~T45（#265）~~ ✅ 2026-09-03 完成（608db3d，PR #313，FIX-29a 评论作者 Preload TDD）→ ~~T46（#266）~~ ✅（3c5ed7c，PR #314，评论回复/编辑/删除 UI+分页）→ {~~T47（#267）~~ ✅（1b107b3，PR #324，高踩比折叠阈值经配置下发）、~~T54（#274）~~ ✅（b719c43，PR #315，举报入口含 409 提示）}；~~T23（#243）~~ ✅（55f3647，PR #312，38 错误码 zh/en 映射统一出口）、~~T25（#245）~~ ✅（33540af，PR #317，发布配置消费化 /config/public）、~~T35（#255）~~ ✅ 2026-09-03 完成（acaeba0，PR #307：用户主页 MessageComposeButton 发起首条私信，纯前端零后端 diff；单测 486 绿 + 隔离栈 rig 10/10 含首条 201/连续第二条 403 行内提示；道 2 顺带收口；遗留 ChatWindow 坏 i18n key 已由 PR #308 修复、NLSearch 无消费者开清理票 #309）、T36（#256）、T53（#273）、T55（#275）
...
### 3.2 RAG 行为冻结点护栏（2026-09-02 修订二新增）

- **A-04（#286）收口 = 检索行为冻结点**。此后任何触及 chunker、embedding 模型/版本、检索融合参数、rerank、查询扩展的新票或增项：要么移到 A-04 之前执行；要么票面标注「RAG 行为变更」，收口时重跑 A-04 消融并升证据文档版本。
- **#204（k6 压测、ops 证据归档、简历数字回填）一律排在冻结点之后**；Agent 级指标（首 token 延迟、流式体验类证据）另须在 A-02/A-06 落地后采集。
- 审计依据（2026-09-02）：当前 69 个执行单元中，A-04 之后无任何触及检索行为的票（A-05/A-06/A-07 与第 8 段均不触及 chunker/embedding/融合）；本护栏为面向未来新增票的预防性条款。
...
- 不新增票据：全部执行单元已有票面（#221~#290），本次整合只做范围修订与排序，无需 to-tickets 立新票。**例外（2026-09-02 修订一，用户批准）**：新增语料 v2 票（CORPUS-01，#291，spec=docs/working/2026-09-02-corpus-v2-seed-spec.md），插入第 6 段 A-03 与 A-04 之间；#286（A-04）blocked_by 已增补语料 v2（票面评论注记）。新增票据仅限用户授权的规划会话执行。
