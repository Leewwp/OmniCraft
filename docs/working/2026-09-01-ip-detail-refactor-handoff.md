# IP 详情页「贴吧式社区枢纽」重构 — 交接文档（设计已定稿，待开发原型）

- 创建日期：2026-09-01
- 预计失效日期：2026-11-01（原型交付后归档或转为正式 spec/plan）
- 性质：三轮 grilling 设计讨论的全部决策 + 现状事实索引。**产出物 = 可交互原型，不动生产代码。**
- 下一会话任务：阅读本文档 → 开发 IP 详情页重构原型。
...
5. IP 内搜索：搜索框 + 分组摘要 + 模块 chips 深入 + 三排序。
6. 空态：无 open 提案 CTA（"第一个提案由你发起"）、各模块空态。
7. 静态/模拟数据即可；遵守 design tokens 与 8px 圆角矩形精修方向；所有文案走 i18n 风格的 key 化结构（原型可用假数据）。

## 5. Suggested skills（下一会话 agent 应调用）

- `prototype` — 生成交互原型（主交付）。
- `ui-ux-pro-max` / `web-design-engineer` — 原型视觉实现辅助（token 遵守 design-system.md）。
- 开工前先读：`design/design-system.md`、`design/ui-spec.md` 对应章节、本文档第 1~4 节。

## 6. 明确不做（本阶段）

- 不动生产代码 / 不建迁移 / 不改 routes.go（原型阶段）。
