...
- **✅ 实现已完成（2026-09-02）**：GitHub #290 已 `CLOSED/COMPLETED`，commit 6e6d77e（squash codex/heavy/290）。本文第 1~4 节的设计裁决已全部落地（MVP 记录偏差：新提案通知走 Notify 逐关注者异步扇出而非 outbox 事件；未随票：改动历史时间线 diff 出参、Hub 列表分页 UI）；第 3 节的「现状事实索引」自此过期，仅作历史参考。归档待第 5 段 T17（#237）同窗收口后一并处理。
