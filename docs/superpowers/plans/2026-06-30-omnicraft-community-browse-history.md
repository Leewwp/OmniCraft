...
## Task 7: Follow-Up — Make Destructive Clear Explicit And Cleanup Multi-Replica Safe

> **Review finding (2026-07-16):** the implemented handler treats a missing body and `{ "ids": [] }` as clear-all. That makes an accidentally stripped DELETE body destructive. The scheduler is safe for the current single-server deployment but every future API replica would run the same cleanup timer.

**Files:**
- Modify: `backend/internal/handler/browse_history.go`
- Modify: `backend/internal/handler/browse_history_test.go`
- Modify: `backend/internal/repository/browse_history_repo.go`
- Modify: `backend/internal/repository/browse_history_repo_test.go`
- Modify: `backend/internal/pkg/scheduler/browse_history_cleanup.go`
- Modify: `backend/internal/pkg/scheduler/browse_history_cleanup_test.go`
- Modify: `frontend/app/(protected)/history/page.tsx`
...
- non-empty `ids` deletes only selected rows;
- `clear_all:true` clears all rows;
- missing body, `{}`, and `{ "ids": [] }` return `400 CLEAR_CONFIRMATION_REQUIRED`;
