...
Use fakes in unit tests. Use no-op providers only in explicit local development mode. A Beta release configuration without real providers must return a startup/configuration error. Extend the F-04 captcha config rather than defining a duplicate struct. Add these exact config fields: `web.public_base_url`, `smtp.mode`, `smtp.host`, `smtp.port`, `smtp.user`, `smtp.password`, `smtp.from_address`, `captcha.provider`, `captcha.site_key`, `captcha.secret`, `verification.email_ttl_sec`, `verification.reset_ttl_sec`, `verification.resend_cooldown_sec`, `verification.login_captcha_threshold`, and `verification.password_min_length`. `smtp.mode=logger` and `captcha.provider=bypass` are local-development-only values and must be rejected when `server.mode == "release"`.
