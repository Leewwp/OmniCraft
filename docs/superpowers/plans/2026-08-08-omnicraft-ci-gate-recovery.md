...
- [ ] 在 Ubuntu/Linux 环境复现 `verify-alerts.tests.sh` 的失败，并让失败输出保留 promtool/amtool 根因。
- [ ] 修复容器运行、挂载或平台差异，不降低 Prometheus/Alertmanager 配置校验强度。
- [ ] 本地合同测试与对应 GitHub Actions job 通过；完成规格符合性和代码质量审查。
- [ ] 精确更新 `progress.txt`，独立提交并关闭对应子票。

## Task G2 [heavy]: Restore SBOM and provenance gates

- [ ] 保留 GitHub runner 上 root-owned bind mount 的回归测试或等价 Linux seam，确认修复前失败。
- [ ] 让 Syft 容器以调用者 UID/GID 写入，并提供可写 HOME/TMPDIR；不得放宽 SBOM 内容、确定性或 provenance 校验。
- [ ] `generate-sbom.tests.sh`、实际生成命令与对应 GitHub Actions job 通过；完成两阶段审查。
- [ ] 先检查并保留工作区已有 `scripts/release/generate-sbom.sh` 修改，独立提交并关闭对应子票。

## Task G3 [heavy]: Restore Security gates

- [ ] 用当前存量 fixture 复现 gitleaks `generic-api-key` 误报，同时保留“真实 fake secret 必须失败”的正向检出合同。
- [ ] 使用最窄的路径/指纹 allowlist 或等价机制排除有意测试数据；不得把所有 `*.tests.sh`、`docs/archive/**` 或 generic-api-key 规则整体关闭。
- [ ] `verify-security.tests.sh`、完整 `verify-security.sh -BuildImages` 与对应 GitHub Actions job 通过；完成两阶段审查。
- [ ] 精确更新 `progress.txt`，独立提交并关闭对应子票。
