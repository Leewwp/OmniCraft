...
1. 用户通过顶部导航进入受保护的独立 `/agent` 工作台，以自然语言描述想找的内容或提出与站内内容有关的问题。
2. Agent 选择只读工具检索公开内容。
3. 服务端执行可见性过滤和参数校验。
4. Agent 生成带引用的回答。
5. 用户可从 Agent 引用卡片打开与推荐流共用的内容详情浮层；关闭后恢复原会话滚动位置和引用焦点。直接访问内容 URL 使用完整详情页；失败时降级到普通关键词搜索。
...
- Agent 采用顶部导航进入的独立全页工作台，不再采用 Root Layout 全局右下角聊天挂件；现有 `AgentChatWidget` 是迁移输入而不是目标产品形态。
- 全站搜索保持关键词建议、历史、热搜和普通搜索职责，不提供 Agent 模式切换；自然语言问答统一进入 Agent 工作台。
- 展示流式状态、停止按钮、引用卡片、工具状态摘要、降级提示和重试。
- 引用可键盘访问并明确标识来源；点击后打开共享内容详情浮层，关闭时恢复会话滚动与焦点；回答与引用不能只靠颜色关联。
