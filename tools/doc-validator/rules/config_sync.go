...
func CheckConfigSync() []RuleIssue {
	// Read the current docs/reference/config.md section 7
	archPath := filepath.Join("docs", "reference", "config.md")
...
// SyncConfigFields generates config field table and inserts into docs/reference/config.md.
...
func addAutoGeneratedMarkers(content, startMarker, endMarker, sectionHeading, newSection string) (string, bool) {
	...
	if sectionIdx == -1 {
		fmt.Printf("  warning: section %q not found in docs/reference/config.md, skipping\n", sectionHeading)
		return content, false
	}
