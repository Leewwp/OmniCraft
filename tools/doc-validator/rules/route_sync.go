...
// extractRoutes parses routes.go and extracts all API routes.
func extractRoutes() ([]Route, error) {
	routePath := filepath.Join("backend", "internal", "handler", "routes.go")
...
func CheckRouteSync() []RuleIssue {
	...
	if err != nil {
		return []RuleIssue{{Severity: "ERROR", File: "backend/internal/handler/routes.go", Message: fmt.Sprintf("cannot extract: %v", err)}}
	}
...
func SyncRouteList() error {
	...
	startMarker := "<!-- AUTO-GENERATED: §3.2 API 路由清单 | source: backend/internal/handler/routes.go | DO NOT EDIT MANUALLY -->"
	endMarker := "<!-- END AUTO-GENERATED: §3.2 -->"

	newContent, replaced := replaceBetweenMarkers(string(content), startMarker, endMarker, table)
	if !replaced {
		newContent, _ = addAutoGeneratedMarkers(string(content), startMarker, endMarker, "完整 API 路由清单", table)
	}
